...
log in the local **logs** folder, prints where it is, and starts working. If **checker.exe** is missing, it builds it
right away (with a visible spinner and timeout) so the full EXE experience is ready for this run. If building fails, it
...
The checker runs in **hands-free** mode: scans start immediately, the spinner shows build/download progress, and
auto-fixes proceed without extra keystrokes. Just double-click and watch the log updates.
...
2. If **checker.exe** is present, run it. If not, build it immediately with npm/pkg and a spinner/timeout so you get
   the EXE right away.
3. If the exe build fails or npm/pkg aren’t available, automatically download a portable Node.js runtime (saved under
...
## Files
- `run_checker.cmd` – one-click launcher that logs first, builds **checker.exe** immediately when it can (with a
  spinner), prefers the exe, auto-downloads portable Node, and falls back to batch checks if all else fails.
